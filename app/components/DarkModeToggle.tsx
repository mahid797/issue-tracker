...
const DarkModeToggle = () => {
	...
	if (!mounted) {
		return <Skeleton width="5rem" height="1.7rem" />;
	}

	const isChecked = resolvedTheme === 'light' ? true : false;

	return (
		<>
			<div className="hidden md:block">
				<label className="relative inline-flex cursor-pointer select-none items-center bg-green-200/50 rounded-full">
					<input
						type="checkbox"
						checked={!isChecked}
						onChange={toggleMode}
					...
					<div className="flex peer transition-all ease-in-out duration-500 peer-checked:after:translate-x-full after:absolute after:bg-[#29A383] after:rounded-full after:h-8 after:w-9 after:transition-all ">
						<span
							className={`flex z-10 h-8 w-9 items-center justify-center rounded-full ${
								isChecked ? 'text-white' : 'text-[#29A383]'
							}`}>
							<svg
								width="16"
								height="16"
						...
						<span
							className={`flex z-10 h-8 w-9 items-center justify-center rounded-xl ${
								!isChecked ? 'text-white' : 'text-[#29A383] '
							}`}>
							<svg
								width="16"
			...
			<div className="flex md:hidden h-6 w-6 mb-0.5">
				<input
					id="darkMode"
					type="checkbox"
					checked={!isChecked}
